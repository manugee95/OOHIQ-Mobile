...
export default function SelectSpecialFeature({
	val = "",
	errorMessage = "",
}: {
	val?: string | number;
	...
	const { specialFeaturesRef } = useUploadsStore();
	const queryClient = useQueryClient();
	const [value, setValue] = useState("");

	useEffect(() => {
		const data = queryClient.getQueryData(["special-features"]);
	...
	return (
		<Pressable
			onPress={() => specialFeaturesRef?.current?.present()}
