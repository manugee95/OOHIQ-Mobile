...
export default function VisibilityPoints({
	currentValue,
	setVisibilityPoints,
}: {
	currentValue: string | number;
	setVisibilityPoints: (val: number) => void;
}) {
	const { getCredentials } = useCredentials();
	const sheetRef = useRef<BottomSheetModal | null>(null);
	const { setVisibilityPointsRef, visibilityPointsRef } = useUploadsStore();

	useEffect(() => {
		setVisibilityPointsRef(sheetRef);
	}, []);

	const { data, isLoading } = useQuery({
		queryKey: ["visibility-points"],
		queryFn: async () => {
			const credentials = await getCredentials();
			const res = await ApiInstance.get("/api/visibility-points", {
				// @ts-ignore
			...
			<BottomSheetView className="flex-1">
				<BottomSheetFlashList
					...
					renderItem={({ item }: { item: { id: number; name: string } }) => (
						<Pressable
							onPress={() => {
								setVisibilityPoints(item.id);
								visibilityPointsRef?.current?.dismiss();
							}}
