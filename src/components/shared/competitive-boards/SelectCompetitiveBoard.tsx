...
export default function SelectCompetitiveBoard({
	val = "",
	errorMessage = "",
}: {
	val?: string | number;
	...
	const { noOfCompetitiveBoardsRef } = useUploadsStore();
	const queryClient = useQueryClient();
	const [value, setValue] = useState("");

	useEffect(() => {
		const data = queryClient.getQueryData(["no-of-competitive-boards"]);
	...
	return (
		<Pressable
			onPress={() => noOfCompetitiveBoardsRef?.current?.present()}
