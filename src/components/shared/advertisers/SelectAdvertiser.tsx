...
export default function SelectAdvertiser({
	val = "",
	errorMessage = "",
}: {
	val?: string | number;
	...
	const { advertiserRef } = useRootStore();
	const queryClient = useQueryClient();
	const [value, setValue] = useState("");

	useEffect(() => {
		const data = queryClient.getQueryData(["advertisers"]);
	...
	return (
		<Pressable
			onPress={() => advertiserRef?.current?.present()}
