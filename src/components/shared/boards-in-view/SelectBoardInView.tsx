...
export default function SelectBoardInView({
	val = "",
	errorMessage = "",
}: {
	val?: string | number;
	...
	const { noOfBoardsInViewRef } = useUploadsStore();
	const queryClient = useQueryClient();
	const [value, setValue] = useState("");

	useEffect(() => {
		const data = queryClient.getQueryData(["no-of-boards-in-view"]);
	...
	return (
		<Pressable
			onPress={() => noOfBoardsInViewRef?.current?.present()}
