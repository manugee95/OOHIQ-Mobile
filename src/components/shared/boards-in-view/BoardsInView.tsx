...
export default function BoardsInView({
	currentValue,
	setBoardInView,
}: {
	currentValue: string | number;
	setBoardInView: (val: number) => void;
}) {
	const { getCredentials } = useCredentials();
	const sheetRef = useRef<BottomSheetModal | null>(null);
	const { setNoOfBoardsInViewRef, noOfBoardsInViewRef } = useUploadsStore();

	useEffect(() => {
		setNoOfBoardsInViewRef(sheetRef);
	}, []);

	const { data, isLoading } = useQuery({
		queryKey: ["no-of-boards-in-view"],
		queryFn: async () => {
			const credentials = await getCredentials();
			const res = await ApiInstance.get("/api/no-of-boards-in-view", {
				// @ts-ignore
			...
			<BottomSheetView className="flex-1">
				<BottomSheetFlashList
					...
					renderItem={({ item }: { item: { id: number; name: string } }) => (
						<Pressable
							onPress={() => {
								setBoardInView(item.id);
								noOfBoardsInViewRef?.current?.dismiss();
							}}
