...
export default function PedestrianTraffic({
	currentValue,
	setPedestrianTraffic,
}: {
	currentValue: string | number;
	setPedestrianTraffic: (val: number) => void;
}) {
	const { getCredentials } = useCredentials();
	const pedestrianTrafficSheet = useRef<BottomSheetModal | null>(null);
	const { setPedestrianTrafficRef, pedestrianTrafficRef } = useUploadsStore();

	useEffect(() => {
		setPedestrianTrafficRef(pedestrianTrafficSheet);
	}, []);

	const { data, isLoading } = useQuery({
		queryKey: ["pedestrian-traffics"],
		queryFn: async () => {
			const credentials = await getCredentials();
			const response = await ApiInstance.get("/api/pedestrian-traffic", {
				headers: {
					// @ts-ignore
				...
				<BottomSheetFlashList
					...
					renderItem={({ item }: { item: { id: number; name: string } }) => {
						return (
							<Pressable
								onPress={() => {
									setPedestrianTraffic(item.id);
									pedestrianTrafficRef?.current?.dismiss();
								}}
