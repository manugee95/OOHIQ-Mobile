...
export default function SelectPedestrianTraffic({
	val = "",
	errorMessage = "",
}: {
	val?: string | number;
	...
	const { pedestrianTrafficRef } = useUploadsStore();
	const queryClient = useQueryClient();
	const [value, setValue] = useState("");

	useEffect(() => {
		const data = queryClient.getQueryData(["pedestrian-traffics"]);
	...
	return (
		<Pressable
			onPress={() => pedestrianTrafficRef?.current?.present()}
