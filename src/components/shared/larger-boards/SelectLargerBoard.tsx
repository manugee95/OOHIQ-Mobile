...
export default function SelectLargerBoard({
	val = "",
	errorMessage = "",
}: {
	val?: string | number;
	...
	const { noOfLargerBoardsRef } = useUploadsStore();
	const queryClient = useQueryClient();
	const [value, setValue] = useState("");

	useEffect(() => {
		const data = queryClient.getQueryData(["no-of-larger-boards"]);
	...
	return (
		<Pressable
			onPress={() => noOfLargerBoardsRef?.current?.present()}
