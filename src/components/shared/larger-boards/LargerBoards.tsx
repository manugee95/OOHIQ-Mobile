...
export default function LargerBoards({
	currentValue,
	setNoOfLargerBoards,
}: {
	currentValue: string | number;
	setNoOfLargerBoards: (val: number) => void;
}) {
	const { getCredentials } = useCredentials();
	const sheetRef = useRef<BottomSheetModal | null>(null);
	const { setNoOfLargerBoardsRef, noOfLargerBoardsRef } = useUploadsStore();

	useEffect(() => {
		setNoOfLargerBoardsRef(sheetRef);
	}, []);

	const { data, isLoading } = useQuery({
		queryKey: ["no-of-larger-boards"],
		queryFn: async () => {
			const credentials = await getCredentials();
			const res = await ApiInstance.get("/api/no-of-larger-boards", {
				// @ts-ignore
			...
			<BottomSheetView className="flex-1">
				<BottomSheetFlashList
					...
					renderItem={({ item }: { item: { id: number; name: string } }) => (
						<Pressable
							onPress={() => {
								setNoOfLargerBoards(item.id);
								noOfLargerBoardsRef?.current?.dismiss();
							}}
