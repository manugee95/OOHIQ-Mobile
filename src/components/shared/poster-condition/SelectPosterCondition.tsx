...
export default function SelectPosterCondition({
	val = "",
	errorMessage = "",
}: {
	val?: string | number;
	...
	const { posterConditionRef } = useRootStore();
	const queryClient = useQueryClient();
	const [value, setValue] = useState("");

	useEffect(() => {
		const data = queryClient.getQueryData(["poster-conditions"]);
	...
	return (
		<Pressable
			onPress={() => posterConditionRef?.current?.present()}
