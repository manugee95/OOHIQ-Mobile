...
export default function PosterConditions({
	currentValue,
	setPosterCondition,
}: {
	currentValue: string | number;
	setPosterCondition: (val: number) => void;
}) {
	const { getCredentials } = useCredentials();
	const posterConditionSheet = useRef<BottomSheetModal | null>(null);
	const { setPosterConditionRef, posterConditionRef } = useRootStore();

	useEffect(() => {
		setPosterConditionRef(posterConditionSheet);
	}, []);

	const { data, isLoading } = useQuery({
		queryKey: ["poster-conditions"],
		queryFn: async () => {
			const credentials = await getCredentials();
			const response = await ApiInstance.get("/api/poster-condition", {
				headers: {
					// @ts-ignore
				...
				<BottomSheetFlashList
					...
					renderItem={({ item }: { item: { id: number; name: string } }) => {
						return (
							<Pressable
								onPress={() => {
									setPosterCondition(item.id);
									posterConditionRef?.current?.dismiss();
								}}
