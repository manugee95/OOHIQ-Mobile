...
export default function SelectDistanceOfVisibility({
	val = "",
	errorMessage = "",
}: {
	val?: string | number;
	...
	const { distanceOfVisibilityRef } = useUploadsStore();
	const queryClient = useQueryClient();
	const [value, setValue] = useState("");

	useEffect(() => {
		const data = queryClient.getQueryData(["distance-of-visibility"]);
	...
	return (
		<Pressable
			onPress={() => distanceOfVisibilityRef?.current?.present()}
