...
export default function SelectBoardLevel({
	val = "",
	errorMessage = "",
}: {
	val?: string | number;
	...
	const { boardLevelRef } = useUploadsStore();
	const queryClient = useQueryClient();
	const [value, setValue] = useState("");

	useEffect(() => {
		const data = queryClient.getQueryData(["board-level"]);
	...
	return (
		<Pressable
			onPress={() => boardLevelRef?.current?.present()}
