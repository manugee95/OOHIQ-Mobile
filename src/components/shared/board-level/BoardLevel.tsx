...
export default function BoardLevel({
	currentValue,
	setBoardLevel,
}: {
	currentValue: string | number;
	setBoardLevel: (val: number) => void;
}) {
	const { getCredentials } = useCredentials();
	const distanceOfVisibilitySheet = useRef<BottomSheetModal | null>(null);
	const { setBoardLevelRef, boardLevelRef } = useUploadsStore();

	useEffect(() => {
		setBoardLevelRef(distanceOfVisibilitySheet);
	}, []);

	const { data, isLoading } = useQuery({
		queryKey: ["board-level"],
		queryFn: async () => {
			const credentials = await getCredentials();
			const response = await ApiInstance.get("/api/board-level", {
				headers: {
					// @ts-ignore
				...
				<BottomSheetFlashList
					...
					renderItem={({ item }: { item: { id: number; name: string } }) => {
						return (
							<Pressable
								onPress={() => {
									setBoardLevel(item.id);
									boardLevelRef?.current?.dismiss();
								}}
