...
export default function SelectBoardPositioning({
	val = "",
	errorMessage = "",
}: {
	val?: string | number;
	...
	const { boardPositioningRef } = useUploadsStore();
	const queryClient = useQueryClient();
	const [value, setValue] = useState("");

	useEffect(() => {
		const data = queryClient.getQueryData(["board-positioning"]);
	...
	return (
		<Pressable
			onPress={() => boardPositioningRef?.current?.present()}
