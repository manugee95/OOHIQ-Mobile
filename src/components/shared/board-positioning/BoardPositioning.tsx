...
export default function BoardPositioning({
	currentValue,
	setBoardPositioning,
}: {
	currentValue: string | number;
	setBoardPositioning: (val: number) => void;
}) {
	const { getCredentials } = useCredentials();
	const distanceOfVisibilitySheet = useRef<BottomSheetModal | null>(null);
	const { setBoardPositioningRef, boardPositioningRef } = useUploadsStore();

	useEffect(() => {
		setBoardPositioningRef(distanceOfVisibilitySheet);
	}, []);

	const { data, isLoading } = useQuery({
		queryKey: ["board-positioning"],
		queryFn: async () => {
			const credentials = await getCredentials();
			const response = await ApiInstance.get("/api/board-positioning", {
				headers: {
					// @ts-ignore
				...
				<BottomSheetFlashList
					...
					renderItem={({ item }: { item: { id: number; name: string } }) => {
						return (
							<Pressable
								onPress={() => {
									setBoardPositioning(item.id);
									boardPositioningRef?.current?.dismiss();
								}}
