...
export default function TrafficSpeeds({
	currentValue,
	setTrafficSpeed,
}: {
	currentValue: string | number;
	setTrafficSpeed: (val: number) => void;
}) {
	const { getCredentials } = useCredentials();
	const posterConditionSheet = useRef<BottomSheetModal | null>(null);
	const { setTrafficSpeedRef, trafficSpeedRef } = useRootStore();

	useEffect(() => {
		setTrafficSpeedRef(posterConditionSheet);
	}, []);

	const { data, isLoading } = useQuery({
		queryKey: ["traffic-speed"],
		queryFn: async () => {
			const credentials = await getCredentials();
			const response = await ApiInstance.get("/api/traffic-speed", {
				headers: {
					// @ts-ignore
				...
				<BottomSheetFlashList
					...
					renderItem={({ item }: { item: { id: number; name: string } }) => {
						return (
							<Pressable
								onPress={() => {
									setTrafficSpeed(item.id);
									trafficSpeedRef?.current?.dismiss();
								}}
