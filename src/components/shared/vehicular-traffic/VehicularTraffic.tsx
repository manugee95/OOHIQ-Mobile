...
export default function VehicularTraffic({
	currentValue,
	setVehicularTraffic,
}: {
	currentValue: string | number;
	setVehicularTraffic: (val: number) => void;
}) {
	const { getCredentials } = useCredentials();
	const vehicularTrafficSheet = useRef<BottomSheetModal | null>(null);
	const { setVehicularTrafficRef, vehicularTrafficRef } = useUploadsStore();

	useEffect(() => {
		setVehicularTrafficRef(vehicularTrafficSheet);
	}, []);

	const { data, isLoading } = useQuery({
		queryKey: ["vehicular-traffics"],
		queryFn: async () => {
			const credentials = await getCredentials();
			const response = await ApiInstance.get("/api/vehicular-traffic", {
				headers: {
					// @ts-ignore
				...
				<BottomSheetFlashList
					...
					renderItem={({ item }: { item: { id: number; name: string } }) => {
						return (
							<Pressable
								onPress={() => {
									setVehicularTraffic(item.id);
									vehicularTrafficRef?.current?.dismiss();
								}}
