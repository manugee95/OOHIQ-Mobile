...
export default function SelectVehicularTraffic({
	val = "",
	errorMessage = "",
}: {
	val?: string | number;
	...
	const { vehicularTrafficRef } = useUploadsStore();
	const queryClient = useQueryClient();
	const [value, setValue] = useState("");

	useEffect(() => {
		const data = queryClient.getQueryData(["vehicular-traffics"]);
	...
	return (
		<Pressable
			onPress={() => vehicularTrafficRef?.current?.present()}
